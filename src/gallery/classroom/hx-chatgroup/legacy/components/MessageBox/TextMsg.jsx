...
export const TextMsg = ({ item }) => {
  const { apis, roomId, roomUuid, loginUser, roleType, loginNickName } = useShallowEqualSelector((state) => {
    return {
      apis: state?.apis,
  ...
  // 删除消息
  const deleteMsg = async (recallId) => {
    await apis.messageAPI.removeMsg(recallId)
